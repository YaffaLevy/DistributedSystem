...
public class SlaveB {
    ...
    public static void main(String[] args) throws IOException {
        try (Socket socketSlaveB = new Socket("127.0.0.1", 30123);
             BufferedReader inputB = new BufferedReader(new InputStreamReader(socketSlaveB.getInputStream()));
             PrintWriter output = new PrintWriter(socketSlaveB.getOutputStream(), true)) {

            while (true) {
                System.out.println("slaveB is running");
            }
            /*ArrayList<String> receivedJobList = new ArrayList<>();

            // Receive packets until DONE is received
            String slaveBJobs;
            slaveBJobs = inputB.readLine();
            System.out.println("Received: " + slaveBJobs);

            while (!(slaveBJobs = inputB.readLine()).equals("SlaveA")) {
                receivedJobList.add(slaveBJobs);
                System.out.println("Received: " + slaveBJobs);
            }
